...
// const ADMIN_FEE  = 5000; // ← tambahkan ini
...
//             { label: 'Biaya Admin', value: 'Rp 5.000 + 2.5%' }, // ← tambahkan ini
//           ].map(r => (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-none p-4 md:p-8 shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-6">
          {[
            { label: 'Min. Tarik',  value: `Rp ${MIN_TARIK.toLocaleString('id-ID')}` },
            { label: 'Maks. Tarik', value: `Rp ${(MAX_TARIK / 1000000).toFixed(0)}jt` },
            { label: 'Biaya Admin', value: 'Rp 5.000 + 2.5%' },
          ].map(r => (
            <div key={r.label} className="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-none p-3 text-center">
                      ...
                      <tr key={wd._id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-all">
                        <td className="px-6 py-5">
                          <p className="flex items-center text-sm font-black text-slate-800 dark:text-green-400">
                            {Number(wd.amount).toLocaleString('id-ID')} <ArrowRight size={14} className='mx-1' /> {Number(wd.amount - (wd.amount * 0.025) - 5000).toLocaleString('id-ID')}
                          </p>
                        </td>
                        <td className="px-6">
                          <p className="text-[14px] relative top-[-1.4px] text-slate-400 dark:text-red-300 font-medium">2.5% + Rp 5.000</p>
                          {/* <p className="font-bold text-slate-600 dark:text-slate-300 text-sm">{wd.accountName}</p> */}
                        </td>
                        <td className="px-6 py-5">
                          <p className="font-bold text-slate-600 dark:text-slate-300 text-sm">{wd.paymentMethod || 'BANK'}</p>
