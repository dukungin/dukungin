...
const InstantTestAlert = ({ overlayToken, settings, user }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-none p-4 md:p-6 shadow-xs border border-slate-100 dark:border-slate-800 space-y-5">
      ...
      {/* Voice Recorder — sama seperti di SupporterPage */}
      <div className="space-y-2">
        <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block">
          Pesan Suara{' '}
          <span className="normal-case font-medium text-slate-300 dark:text-slate-600">(opsional)</span>
        </label>
        <VoiceRecorder
        ...
        {voiceUrl && (
          <div className="flex items-center gap-3 p-3 bg-rose-50 dark:bg-rose-950/30 rounded-none border border-rose-100 dark:border-rose-900">
            ...
            <button
              onClick={() => setVoiceUrl('')}
              className="ml-auto text-slate-400 hover:text-red-400 transition-colors text-xs font-bold cursor-pointer"
            >
              Hapus
            </button>
          </div>
      ...
      <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium text-center">
        ⚠️ Pastikan OBS overlay sudah dibuka di browser source sebelum test
      </p>
    </div>
  );
};
