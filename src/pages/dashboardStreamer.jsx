...
//   const [isFullscreen, setIsFullscreen] = useState(false); // ← tambah ini
...
//             background: 'linear-gradient(155deg,#1a1a2e 0%,#0d0d1a 60%,#12121f 100%)'
//           }}>
//             {/* Watermark */}
//             <span className="absolute inset-0 flex items-center justify-center text-[clamp(60px,15vw,180px)] font-black text-white/[0.02] pointer-events-none select-none" style={{ letterSpacing: -8 }}>
//               LIVE
//             </span>

//             {/* Fake stream UI — top bar */}
//             <div className="absolute top-0 left-0 right-0 flex items-center gap-3 px-6 py-4"
...
//             {/* Overlay Alert — sama dengan mini preview tapi di posisi asli */}
...
//                     style={{
//                       position: 'absolute',
//                       ...posMap[settings.overlayPosition || 'bottom-right'],
//                       zIndex: 10,
//                     }}>
//                     {renderAlert()}
//                   </motion.div>
          ...
          <div key={u._id} className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm flex flex-col gap-4">
            ...
            {showFollowBtn && u._id !== currentUserId && (
              <button onClick={() => toggleMutation.mutate(u._id)} disabled={toggleMutation.isPending}
                className={`cursor-pointer active:scale-[0.97] hover:brightness-95 w-full py-3 rounded-xl font-black text-xs transition-all disabled:opacity-60 ${
                  u.isFollowing ? 'bg-slate-100 text-slate-500 hover:bg-red-50 hover:text-red-500' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-100'
                }`}>
                {u.isFollowing ? 'Unfollow' : '+ Follow'}
              </button>
            )}
                    ...
                    {[
                      ...
                      return (
                        <div key={path} className="flex items-center gap-4 bg-white rounded-xl p-4 border border-slate-300">
                          ...
                          <button onClick={() => copyToClipboard(widgetUrl)}
                            className="cursor-pointer active:scale-[0.97] p-2.5 bg-slate-200 hover:bg-indigo-100 hover:text-indigo-600 text-slate-500 rounded-xl transition-all flex-shrink-0">
                            <Copy size={15} />
                          </button>
                        </div>
