...
//               className={`cursor-pointer active:scale-[0.99] text-left p-4 rounded-xl border-2 transition-all space-y-1.5 ${
//                 localAction === opt.id ? opt.active + ' shadow-md' : 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-600'
//               }`}>
//               <div className="flex items-center gap-2">
//                 <span className="text-xl">{opt.emoji}</span>
...
//                 {words.map(word => (
//                   <span key={word}
//                     className="md:w-max w-[48.9%] flex justify-center md:justify-start items-center gap-2 px-4 py-2 bg-red-50 dark:bg-red-950/40 text-red-600 dark:text-red-400 rounded-xl text-sm font-black border border-red-100 dark:border-red-900">
//                     {word}
...
//         Tampilkan progress target donasi di halaman publik kamu.
//       </p>
//       {isLoading
//         ? <div className="text-slate-400 text-sm font-bold animate-pulse">Memuat...</div>
//         : (
...
//                 <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
//                   <div className="flex flex-col gap-1">
//                     <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Judul Milestone</label>
//                     <input value={m.title} placeholder="contoh: Beli mic baru!"
//                       onChange={e => upd(i, 'title', e.target.value)}
...
//                   </div>
//                   <div className="flex flex-col gap-1">
//                     <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Target (Rp)</label>
//                     <input type="number" value={m.targetAmount}
//                       onChange={e => upd(i, 'targetAmount', Number(e.target.value))}
//                       className="w-full p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-sm text-slate-900 dark:text-slate-100 outline-none focus:border-green-400" />
//                   </div>
...
//             ))}
//             <button onClick={add}
//               className="cursor-pointer active:scale-[0.97] w-full py-3 border-2 border-dashed border-green-200 dark:border-green-900 text-green-600 dark:text-green-400 rounded-xl font-black text-sm hover:border-green-400 hover:bg-green-50 dark:hover:bg-green-950/30 transition-all flex items-center justify-center gap-2">
//               <Plus size={16} /> Tambah Milestone
//             </button>
//             {list.length > 0 && (
//               <button onClick={() => mutation.mutate(list)} disabled={mutation.isPending}
...
//           <button key={m.id} onClick={() => setMode(m.id)}
//             className={`cursor-pointer active:scale-[0.97] px-4 py-2 rounded-xl font-black text-xs transition-all ${
//               mode === m.id ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700'
//             }`}>
//             {m.label}
//           </button>
//         ))}
...
//           <button onClick={() => onChange('')}
//             className={`cursor-pointer active:scale-[0.97] flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 font-black text-xs transition-all ${
//               !value ? 'border-slate-600 bg-slate-800 text-white shadow-md' : 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-400 hover:border-slate-300'
//             }`}>
//             <span className="text-lg">🔇</span>
//             <span>Tanpa Suara</span>
//           </button>
...
//               className={`cursor-pointer active:scale-[0.97] flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 font-black text-xs transition-all ${
//                 value === preset.url
//                   ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 shadow-md shadow-indigo-100'
//                   : 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-500 hover:border-slate-300'
//               }`}>
//               <span className="text-lg">{preset.label.split(' ')[1]}</span>
//               <span>{preset.label.split(' ')[0]}</span>
//               <span onClick={e => { e.stopPropagation(); playPreview(preset.url); }}
//                 className="text-[9px] font-medium text-slate-400 hover:text-indigo-600 transition-colors">▶ preview</span>
//             </button>
//           ))}
//         </div>
//       )}
...
//           <p className="text-[10px] text-slate-400 font-medium ml-1 italic">*Paste URL file audio dari Cloudinary, Google Drive, atau hosting lainnya</p>
//         </div>
//       )}
//       {value && (
...
//           <button onClick={() => playPreview(value)}
//             className="cursor-pointer active:scale-[0.97] w-8 h-8 bg-indigo-600 rounded-xl flex items-center justify-center text-white text-xs hover:bg-indigo-700 transition-all flex-shrink-0">▶</button>
//           <div className="flex-1 min-w-0">
...
//   const remove = (i) => onChange(tiers.filter((_, idx) => idx !== i));
//   const upd    = (i, key, val) => onChange(tiers.map((t, idx) =>
//     idx === i ? { ...t, [key]: key === 'minAmount' || key === 'maxAmount' ? (val === '' ? null : Number(val)) : val } : t
//   ));
...
//       <div className="flex flex-col items-start gap-4">
//         <div className="p-4 bg-white rounded-3xl border-4 border-slate-900 shadow-xl inline-block">
//           <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(donateUrl)}&color=0f172a&bgcolor=ffffff&format=svg&margin=0`}
//             alt="QR Code" width={200} height={200} />
//         </div>
...
//       <div className="grid grid-cols-2 gap-3">
//         <button onClick={copy}
//           className={`cursor-pointer active:scale-[0.97] flex items-center justify-center gap-2 py-4 rounded-xl font-black text-sm transition-all ${copied ? 'bg-green-500 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700'}`}>
//           {copied ? <><CheckCircle2 size={16} /> Tersalin!</> : <><Copy size={16} /> Salin URL</>}
//         </button>
...
//         <div className="w-9 h-9 bg-amber-500 rounded-xl flex items-center justify-center text-lg">🏆</div>
//         <div>
//           <p className="font-black text-slate-800 dark:text-slate-100">Leaderboard Donor</p>
//           <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Semua waktu</p>
//         </div>
//       </div>
//       <div className="p-6 space-y-3">
//         {topDonors.map((donor, i) => (
//           <motion.div key={donor.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
//             className={`flex items-center gap-4 p-4 rounded-xl ${i < 3 ? 'bg-gradient-to-r ' + colors[i] + ' text-white' : 'bg-slate-50 dark:bg-slate-800'}`}>
...
//             </div>
//             <div className="flex-1 min-w-0">
//               <p className={`font-black text-sm truncate ${i < 3 ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}>{donor.name}</p>
//               <p className={`text-[10px] font-medium ${i < 3 ? 'text-white/70' : 'text-slate-400 dark:text-slate-500'}`}>{donor.count}x donasi</p>
//             </div>
//             <p className={`font-black text-sm flex-shrink-0 ${i < 3 ? 'text-white' : 'text-indigo-600 dark:text-indigo-400'}`}>
//               Rp {Number(donor.totalAmount).toLocaleString('id-ID')}
//             </p>
//           </motion.div>
//         ))}
...
//             <p className="text-slate-400 text-xs font-black uppercase tracking-widest mb-1">Super Admin</p>
//             <h2 className="text-2xl font-black">Manajemen Penarikan Dana</h2>
//             <p className="text-slate-400 text-sm font-medium mt-1">Approve = kamu sudah transfer manual. Reject = saldo dikembalikan ke streamer.</p>
//           </div>
//           <div className="flex items-center gap-2 text-slate-400 text-xs font-bold">
//             <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /> Auto 30s
//             <button onClick={() => refetch()} disabled={isFetching} className="ml-1 hover:text-white transition-colors disabled:opacity-50">
...
//                         <td className="px-6 py-5">
//                           <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black ${
//                             wd.status === 'COMPLETED' ? 'bg-green-100 dark:bg-green-950/40 text-green-600 dark:text-green-400'
//                             : wd.status === 'FAILED'   ? 'bg-red-100 dark:bg-red-950/40 text-red-500 dark:text-red-400'
...
//                           {wd.status === 'PENDING' && (
//                             <div className="flex gap-2">
//                               <button onClick={() => { setSelectedId(wd._id); setShowApproveModal(true); }}
//                                 className="cursor-pointer px-2.5 py-2 bg-green-100 dark:bg-green-950/40 text-green-600 dark:text-green-400 rounded-lg text-sm font-black hover:bg-green-200 transition-all flex items-center">
//                                 <Check size={18} />
//                               </button>
//                               <button onClick={() => { setSelectedId(wd._id); setRejectNote(''); setShowRejectModal(true); }}
//                                 className="cursor-pointer px-2.5 py-2 bg-red-50 dark:bg-red-950/40 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900 rounded-lg text-sm font-black hover:bg-red-100 transition-all">
//                                 <X size={18} />
//                               </button>
//                             </div>
//                           )}
...
//                         className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-8 text-center border border-slate-100 dark:border-slate-800"
//                         onClick={e => e.stopPropagation()}>
...
//                             className="cursor-pointer flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-black rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">Batal</button>
//                           <button onClick={() => updateMutation.mutate({ id: selectedId, status: 'COMPLETED' })} disabled={updateMutation.isPending}
//                             className="cursor-pointer flex-1 py-4 bg-green-600 hover:bg-green-700 text-white font-black rounded-2xl transition-all disabled:opacity-70">
//                             {updateMutation.isPending ? 'Memproses...' : 'Ya, Sudah Transfer'}
//                           </button>
//                         </div>
//                       </motion.div>
...
//                       onClick={() => setShowRejectModal(false)}>
//                       <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
//                         className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-8 border border-slate-100 dark:border-slate-800"
//                         onClick={e => e.stopPropagation()}>
...
//                           placeholder="Contoh: Rekening tidak valid, saldo tidak cukup..."
//                           className="w-full h-32 p-4 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-2xl focus:border-red-400 outline-none resize-y font-medium" />
//                         <div className="flex gap-3 mt-6">
//                           <button onClick={() => setShowRejectModal(false)}
//                             className="cursor-pointer flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-black rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">Batal</button>
//                           <button onClick={() => updateMutation.mutate({ id: selectedId, status: 'FAILED', note: rejectNote || 'Ditolak oleh admin' })} disabled={updateMutation.isPending}
//                             className="cursor-pointer flex-1 py-4 bg-red-600 hover:bg-red-700 text-white font-black rounded-2xl transition-all disabled:opacity-70">
//                             {updateMutation.isPending ? 'Memproses...' : 'Konfirmasi Tolak'}
//                           </button>
//                         </div>
//                       </motion.div>
...
//     { value: 'image', icon: <ImageIcon size={13} />, label: 'Gambar', desc: 'jpg, gif, png, webp' },
//     { value: 'video', icon: <Video size={13} />, label: 'Video', desc: 'mp4, webm, mov' },
//     { value: 'both',  icon: <span className="flex items-center gap-0.5"><ImageIcon size={11} /><span className="text-[8px] font-black opacity-60">+</span><Video size={11} /></span>, label: 'Keduanya', desc: 'gambar & video' },
//   ];
...
//           <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center mx-auto mb-3">
//             <ImageIcon size={18} className="text-slate-400" />
//           </div>
//           <p className="text-sm font-black text-slate-500 dark:text-slate-400">Belum ada ketentuan media</p>
//           <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium mt-1 leading-relaxed">Tambahkan agar donor bisa menyertakan gambar atau video.</p>
//         </div>
//       )}
//       {triggers.map((t, i) => (
...
//             </div>
//             <div className="flex flex-col gap-1.5">
//               <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Nominal Minimum (Rp)</label>
//               <div className="relative">
//                 <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-black text-slate-400">Rp</span>
//                 <input type="number" value={t.minAmount} onChange={e => update(i, 'minAmount', Number(e.target.value))}
//                   className="w-full p-3 pl-9 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-sm text-slate-900 dark:text-slate-100 outline-none focus:border-indigo-400 transition-all" />
//               </div>
//             </div>
//           </div>
//           <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
//             {mediaTypeOptions.map(opt => (
//               <button key={opt.value} onClick={() => update(i, 'mediaType', opt.value)}
//                 className={`cursor-pointer active:scale-[0.97] flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 font-black text-xs transition-all ${
...
//     return (
//       <div style={{
//         backgroundColor: theme === 'minimal' ? 'transparent' : bg, color: fg,
//         maxWidth: `${maxW}px`, width: '100%',
//         borderRadius: theme === 'modern' ? 14 : theme === 'classic' ? 4 : 0,
//         overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.45)',
//         border: theme === 'minimal' ? `2px solid ${bg}` : `2px solid ${settings.borderColor || 'rgba(255,255,255,0.15)'}`,
//       }}>
//         {inner}
//       </div>
//     );
...
//             <div className="flex items-center gap-3">
//               <button onClick={triggerDemo}
//                 className="cursor-pointer active:scale-[0.97] flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-black text-xs transition-all">
//                 <span className="w-1.5 h-1.5 bg-red-400 rounded-full animate-pulse" /> Simulasi Donasi
//               </button>
//               <button onClick={() => handleFullScreen()}
...
//                           <td className="px-6 md:px-8 py-5"><p className="text-indigo-600 dark:text-indigo-400 font-black">Rp {Number(item.amount).toLocaleString('id-ID')}</p></td>
//                           <td className="px-6 md:px-8 py-5 max-w-[200px]">
...
//                               item.status === 'PAID'    ? 'bg-green-100 dark:bg-green-950/40 text-green-600 dark:text-green-400'
...
//                   <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
//                     className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-black text-xs hover:bg-slate-200 dark:hover:bg-slate-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed">← Sebelumnya</button>
...
//                   <button onClick={() => setPage(p => Math.min(pagination.totalPages, p + 1))} disabled={page === pagination.totalPages}
//                     className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-black text-xs hover:bg-slate-200 dark:hover:bg-slate-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed">Berikutnya →</button>
//                 </div>
//               )}
//             </>
//           )}
...
//           <button onClick={() => setSearch(searchInput)}
//             className="cursor-pointer active:scale-[0.97] px-6 py-3.5 bg-indigo-600 text-white rounded-xl font-black text-sm hover:bg-indigo-700 transition-all">Cari</button>
//         </div>
//       )}
//       {subTab === 'discover'  && renderUsers(discoverData?.users,  discoverLoading,  true)}
//       {subTab === 'followers' && renderUsers(followersData?.users, followersLoading, false)}
...
//             onClick={() => setShowCopyModal(false)}>
//             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
//               className="bg-white dark:bg-slate-900 rounded-3xl md:max-w-sm max-w-md w-full overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800"
//               onClick={e => e.stopPropagation()}>
//               <div className="p-4 text-center">
...
//                   Widget <span className="font-bold text-indigo-600 dark:text-indigo-400">{copiedLabel}</span> berhasil disalin ke clipboard.
//                 </p>
//                 <button onClick={() => setShowCopyModal(false)}
//                   className="cursor-pointer hover:brightness-90 w-full py-4 bg-slate-900 dark:bg-slate-700 text-white font-black rounded-2xl transition-all active:scale-[0.99]">
//                   OK, Mengerti
//                 </button>
//               </div>
//             </motion.div>
...
//               className="bg-white dark:bg-slate-900 rounded-3xl max-w-sm w-full overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800"
//               onClick={e => e.stopPropagation()}>
//               <div className="p-8 text-center">
...
//                   Kamu {followAction.type === 'follow' ? 'sekarang mengikuti' : 'tidak lagi mengikuti'}
//                   <span className="font-bold text-indigo-600 dark:text-indigo-400"> @{followAction.username}</span>
//                 </p>
//                 <button onClick={() => setShowFollowModal(false)}
//                   className="cursor-pointer hover:brightness-90 w-full py-4 bg-slate-900 dark:bg-slate-700 text-white font-black rounded-2xl transition-all active:scale-[0.97]">
//                   OK, Mengerti
//                 </button>
//               </div>
//             </motion.div>
...
//                   {toast.isWithdrawal ? (toast.status === 'COMPLETED' ? 'Penarikan Berhasil!' : 'Penarikan Gagal') : 'Donasi Masuk!'}
//                 </span>
//                 <p className="text-slate-700 dark:text-slate-200 text-sm font-medium mt-1">{toast.message}</p>
//               </div>
//               <button onClick={() => setDonationToasts(prev => prev.filter(t => t.id !== toast.id))}
//                 className="text-slate-300 dark:text-slate-600 hover:text-slate-500 transition-colors flex-shrink-0 text-lg leading-none">×</button>
//             </motion.div>
//           ))}
//         </AnimatePresence>
//       </div>
...
//           <header className="flex flex-col md:mt-4 lg:flex-row justify-between items-start lg:items-center z-[-1] gap-8 mb-8 relative">
//             <div className="z-10 md:flex flex-col hidden">
//               <h2 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight leading-none">
//                 {TAB_TITLE[activeTab] || activeTab}
//               </h2>
//               <p className="text-slate-400 dark:text-slate-500 font-medium mt-1">
//                 Selamat datang kembali, <span className="text-slate-800 dark:text-slate-200 font-bold">@{user.username}</span>
//               </p>
//             </div>
//             {profileLoading && (
...
//                                   : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:border-slate-300'
//                               }`}>{t}</button>
//                           ))}
//                         </div>
//                       </div>
...
//                           </div>
//                           <button onClick={() => copyToClipboard(widgetUrl, label)}
//                             className="cursor-pointer active:scale-[0.97] p-2.5 bg-slate-100 dark:bg-slate-700 hover:bg-indigo-100 dark:hover:bg-indigo-950/40 hover:text-indigo-600 text-slate-500 rounded-xl transition-all flex-shrink-0">
//                             <Copy size={15} />
//                           </button>
//                         </div>
//                       );
...
//                       <textarea className="w-full p-5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-xl font-bold outline-none focus:border-indigo-500 h-32 transition-all"
//                         placeholder="Ceritakan tentang kontenmu..." value={profileForm.bio}
//                         onChange={e => setProfileForm(f => ({ ...f, bio: e.target.value }))} />
//                     </div>
//                     <div className="md:col-span-2">
...
const StreamerProfileModal = ({ username, currentUserId, onClose, onFollow }) => {
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }}
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 px-8 pt-8 pb-12 relative overflow-hidden">
            <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center text-white hover:bg-white/30 transition-all cursor-pointer">
              <X size={16} />
            </button>
            <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-white/5 rounded-full" />
            <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-3xl font-black text-slate-800 shadow-xl mb-4">
              {username?.charAt(0).toUpperCase()}
            </div>
            <h2 className="text-2xl font-black text-white">@{username}</h2>
            <p className="text-indigo-200 text-sm font-medium mt-1">Streamer di TapTipTup</p>
          </div>

          <div className="px-8 pt-6 pb-8 space-y-5 -mt-2">
            {isLoading ? (
              <div className="flex items-center justify-center py-8 text-slate-400 gap-3">
                <RefreshCw size={18} className="animate-spin" />
                <span className="font-bold text-sm">Memuat profil...</span>
              </div>
            ) : (
              <>
                {/* Stats */}
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'Followers',  val: streamer?.followersCount ?? '-', color: 'text-indigo-600 dark:text-indigo-400' },
                    { label: 'Following',  val: streamer?.followingCount ?? '-', color: 'text-purple-600 dark:text-purple-400' },
                  ].map(s => (
                    <div key={s.label} className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-4 text-center border border-slate-100 dark:border-slate-700">
                      <p className={`text-2xl font-black ${s.color}`}>{s.val}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest mt-1">{s.label}</p>
                    </div>
                  ))}
                </div>

                {/* Donate link */}
                <div className="flex items-center gap-3 bg-indigo-50 dark:bg-indigo-950/40 rounded-2xl p-4 border border-indigo-100 dark:border-indigo-900">
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Link Donasi</p>
                    <p className="text-xs font-mono text-indigo-600 dark:text-indigo-300 truncate">{donateUrl}</p>
                  </div>
                  <button onClick={() => navigator.clipboard.writeText(donateUrl)}
                    className="cursor-pointer w-8 h-8 bg-indigo-100 dark:bg-indigo-900 rounded-xl flex items-center justify-center text-indigo-600 dark:text-indigo-300 hover:bg-indigo-200 transition-all">
                    <Copy size={13} />
                  </button>
                </div>
                ...
                <div className="flex gap-3">
                  <a href={donateUrl} target="_blank" rel="noreferrer"
                    className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-sm transition-all cursor-pointer active:scale-[0.97]">
                    <ExternalLink size={15} /> Ke Halaman Donasi
                  </a>
                  {onFollow && (
                    <button onClick={onFollow}
                      className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-2xl font-black text-sm transition-all cursor-pointer active:scale-[0.97]">
