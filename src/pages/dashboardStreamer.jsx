...
//   const TAB_TITLE = {
//     settings: 'Dashboard', 
//      alertSettings: 'Alert OBS',      // ← BARU
//     mediaSettings: 'Media Share',  
//     history: 'Riwayat', feeConfig:   'Konfigurasi Fee', wallet: 'Wallet', community: 'Community',
          ...
          <div className="flex gap-2">
            {[{ id: 'received', label: '📥 Donasi Diterima' }, { id: 'sent', label: '📤 Donasi Terkirim' }].map((t) => (
              <button key={t.id} onClick={() => { setHistoryTab(t.id); setPage(1); setStatusFilter(''); }}
                className={`px-5 py-2.5 rounded-2xl font-black text-sm transition-all ${historyTab === t.id ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 hover:border-indigo-200'}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div className="px-6 md:px-10 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-3">
            <div className="flex gap-1.5">
              <button onClick={() => setShowAmounts(v => !v)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-none text-[10px] font-black transition-all border-2 ${showAmounts ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 text-slate-400'}`}>
                {showAmounts ? <Eye size={12} /> : <EyeOff size={12} />} Nominal
              </button>
              <button onClick={() => setShowEmails(v => !v)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-none text-[10px] font-black transition-all border-2 ${showEmails ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 text-slate-400'}`}>
                {showEmails ? <Eye size={12} /> : <EyeOff size={12} />} Email
              </button>
            </div>
            <div className="flex gap-1">
              {[{ val: '', label: 'Semua' }, { val: 'PAID', label: 'PAID' }].map((f) => (
                <button key={f.val} onClick={() => { setStatusFilter(f.val); setPage(1); }}
                  className={`px-3 py-1.5 rounded-none text-[10px] font-black transition-all ${statusFilter === f.val ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 hover:bg-slate-200'}`}>
                  {f.label}
                </button>
              ))}
            </div>
        ...
        <div className="overflow-x-auto">
          {historyTab === 'received' ? (
            <>
              {isLoading ? (
                <div className="flex items-center justify-center py-20 text-slate-400 font-bold gap-3">
                  <div className="w-5 h-5 border-4 border-slate-200 border-t-indigo-600 rounded-none animate-spin" />Memuat riwayat...
                </div>
              ) : (
                <table className="w-full text-left min-w-[700px]">
                  <thead>
                ...
                {sentLoading ? (
                  ...
                  (sentData?.donations || []).map((item) => (
                    <tr key={item._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-all">
                      ...
                      <td className="px-6 md:px-8 py-5 font-black text-indigo-600">Rp {Number(item.amount).toLocaleString('id-ID')}</td>
          ...
          {pagination.totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-400 dark:text-slate-500">Halaman <span className="text-indigo-600 dark:text-indigo-400 font-black">{page}</span> dari {pagination.totalPages}</span>
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2">
        <div className="w-10 h-10 flex-shrink-0 rounded-none overflow-hidden border border-slate-300 dark:border-slate-600 relative group">
          ...
          <div className="absolute inset-0 w-full h-full border-2 border-transparent group-hover:border-indigo-400 transition-all" style={{ backgroundColor: pickerHex }} aria-hidden="true" />
        </div>
        <input id={inputId} name={inputId} type="text" value={raw} onChange={handleTextChange} onBlur={handleTextBlur}
          ...
          aria-label={`${label} hex value`} />
        <div className="flex-1 h-10 rounded-none border border-slate-200 dark:border-slate-700 cursor-pointer hover:shadow-md hover:border-indigo-300 transition-all" style={{ backgroundColor: previewColor }} title={previewColor} aria-hidden="true" />
      </div>
        ...
        <AnimatePresence>
          {donationToasts.map(t => (
            <motion.div key={t.id} initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }}
              ...
              <div className="flex-1 min-w-0">
                <span className={`text-[10px] font-black uppercase tracking-widest ${t.isWithdrawal ? (t.status === 'COMPLETED' ? 'text-green-600 dark:text-green-400' : 'text-red-500') : 'text-indigo-600 dark:text-indigo-400'}`}>
                    ...
                    <div className="mt-8 space-y-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-4 md:grid-cols-6 gap-2">
                          {ICON_PRESETS.map(({ emoji, label }) => (
                            <button key={emoji} onClick={() => upd('customIcon', emoji === '💜' ? '' : emoji)} title={label}
                              className={`flex flex-col items-center gap-1 p-3 rounded-none border-2 text-lg transition-all cursor-pointer active:scale-[0.95] ${(settings.customIcon || '💜') === emoji || (!settings.customIcon && emoji === '💜') ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-950/40' : 'border-slate-100 dark:border-slate-700 hover:border-slate-300 bg-slate-50 dark:bg-slate-800'}`}>
                              <span>{emoji}</span>
                              <span className="text-[8px] font-black text-slate-400 leading-none">{label}</span>
                            </button>
                          ))}
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-none p-4 md:p-6 shadow-xs border border-slate-100 dark:border-slate-800">
                    {[
                      ...
                      <div key={label} className="bg-slate-100 dark:bg-slate-800 p-6 rounded-none border-2 border-dashed border-slate-200 dark:border-slate-700 mb-8">
                        ...
                        <div className="flex gap-3">
                          <input readOnly value={url} className="flex-1 bg-transparent font-mono text-sm text-indigo-600 dark:text-indigo-400 font-bold outline-none overflow-hidden text-ellipsis" />
                          <button onClick={() => copyToClipboard(url)} className="text-slate-400 hover:text-indigo-600 cursor-pointer active:scale-[0.98]"><Copy size={18} /></button>
                        </div>
                      </div>
                    ))}
                    <button onClick={() => saveSettingsMutation.mutate(settings)} disabled={saveSettingsMutation.isPending}
