              ...
              <table className="w-full text-left min-w-[900px]">
                ...
                <tbody className="divide-y divide-slate-50">
                  {withdrawals.length === 0
                    ? <tr><td colSpan={8} className="text-center py-16 text-slate-400 font-bold">Tidak ada request penarikan</td></tr>
                    : withdrawals.map(wd => (
...
// ─── MediaTriggersEditor (SIMPLIFIED) ────────────────────────────────────────
//
// Streamer HANYA mengisi:
//   • Nominal Minimum  → batas minimal donasi agar input media muncul di SupporterPage
//   • Tipe Media       → apakah donor boleh kirim 'image', 'video', atau 'both'
//   • Label            → nama trigger yang ditampilkan ke donor (opsional)
//
// Tidak ada input URL media di sini — URL diisi oleh donor saat donasi.
//
const MediaTriggersEditor = ({ triggers, onChange }) => {
  ...
  // Label icon & deskripsi per mediaType
  const mediaTypeOptions = [
    {
      value: 'image',
      icon: <ImageIcon size={13} />,
      label: 'Gambar',
      desc: 'jpg, gif, png, webp',
    },
    {
      value: 'video',
      icon: <Video size={13} />,
    ...
    },
    {
      ...
      icon: (
        <span className="flex items-center gap-0.5">
          <ImageIcon size={11} /><span className="text-[8px] font-black opacity-60">+</span><Video size={11} />
        </span>
      ),
      label: 'Keduanya',
      desc: 'gambar & video',
  ...
  return (
    <div className="space-y-4">
      {/* Penjelasan singkat di atas list */}
      {triggers.length === 0 && (
      ...
      {triggers.map((t, i) => (
        <div key={i} className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => remove(i)} className="cursor-pointer active:scale-[0.97] text-red-400 hover:text-red-600 transition-colors p-1">
              <Trash2 size={15} />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                Label <span className="normal-case font-medium text-slate-300">(opsional)</span>
              </label>
              <input
                value={t.label}
                placeholder="contoh: Sultan Alert, Fan Art Alert"
              ...
              />
              <p className="text-[9px] text-slate-400 font-medium ml-0.5">
                Nama ini yang dilihat donor saat donasi
              </p>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  type="number"
                  value={t.minAmount}
                  onChange={e => update(i, 'minAmount', Number(e.target.value))}
              ...
              <p className="text-[9px] text-slate-400 font-medium ml-0.5">
                Input media muncul saat donasi ≥ nominal ini
              </p>
            </div>
          </div>

          {/* Tipe Media — 3 pilihan tombol */}
          <div className="flex flex-col gap-2">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
              Tipe Media yang Diizinkan Donor
            </label>
            <div className="grid grid-cols-3 gap-2">
              {mediaTypeOptions.map(opt => (
                <button
                  key={opt.value}
                  onClick={() => update(i, 'mediaType', opt.value)}
                  className={`cursor-pointer active:scale-[0.97] flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 font-black text-xs transition-all ${
                    t.mediaType === opt.value
                      ...
                      : 'border-slate-100 text-slate-400 hover:border-slate-300 hover:bg-white'
                  }`}
                >
                  <span className={t.mediaType === opt.value ? 'text-indigo-600' : 'text-slate-400'}>
                    {opt.icon}
                  </span>
                  <span>{opt.label}</span>
                  <span className={`text-[9px] font-medium ${t.mediaType === opt.value ? 'text-indigo-400' : 'text-slate-300'}`}>
                    {opt.desc}
                  </span>
                </button>
              ))}
          ...
          {/* Info box — apa yang akan terjadi */}
          <div className="rounded-xl bg-indigo-50/70 border border-indigo-100 px-4 py-3 flex items-start gap-3">
            ...
            <p className="text-[11px] text-indigo-700 font-medium leading-relaxed">
              Saat donor menginput nominal{' '}
              <strong>≥ Rp {Number(t.minAmount).toLocaleString('id-ID')}</strong>,
              akan muncul input{' '}
              <strong>
                {t.mediaType === 'image' ? 'link gambar' : t.mediaType === 'video' ? 'link video' : 'link gambar atau video'}
              </strong>{' '}
              di halaman donasi.{' '}
              Media yang dikirim donor akan tampil di overlay OBS kamu.
            </p>
          </div>
        </div>
      ))}
                  ...
                  <div className="mt-4 mb-6 rounded-2xl bg-purple-50 border border-purple-100 px-5 py-4 space-y-2">
                    ...
                    <ul className="text-[11px] text-purple-600 font-medium space-y-1.5 leading-relaxed">
                      <li className="flex items-start gap-2">
                        ...
                        <span>Kamu tentukan <strong>nominal minimum</strong> dan <strong>tipe media</strong> yang diizinkan (gambar / video / keduanya).</span>
                      </li>
                      <li className="flex items-start gap-2">
                        <span className="mt-0.5 flex-shrink-0">2.</span>
                        <span>Saat donor memasukkan nominal yang memenuhi syarat, <strong>input link media muncul otomatis</strong> di halaman donasi mereka.</span>
                      </li>
                      <li className="flex items-start gap-2">
                        <span className="mt-0.5 flex-shrink-0">3.</span>
                        <span>Donor isi link gambar/video milik mereka sendiri — yang kemudian <strong>tampil di overlay OBS-mu</strong> saat donasi masuk.</span>
                      </li>
                    </ul>
                  </div>
                ...
                : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[700px]">
                      <thead>
                        <tr className="bg-slate-200/50 border-b border-slate-100 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                          {['Donatur','Jumlah','Pesan','Status'].map((h, i) => <th key={h} className={`px-10 py-6${i === 3 ? ' text-center' : ''}`}>{h}</th>)}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {history.length === 0
                          ? <tr><td colSpan={4} className="text-center py-16 text-slate-400 font-bold">Belum ada donasi</td></tr>
