...
import { formatDate } from '../pages/DashboardStreamer'; // atau taruh helper di tempat yang sama
  ...
  return (
    <div className="space-y-6 pb-6">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
        ...
        {isLoading ? (
          <div className="py-20 text-center text-slate-400">Memuat riwayat donasi...</div>
        ) : donations.length === 0 ? (
          <div className="py-20 text-center text-slate-400">
            <p className="text-5xl mb-4">💸</p>
            <p className="font-black">Belum ada donasi yang kamu kirim</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left min-w-[700px]">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {donations.map((donation) => (
                  <tr key={donation._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-6 py-5">
                      <p className="font-black">@{donation.streamerUsername}</p>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5">
                      <span className="px-3 py-1 bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-400 text-xs font-black rounded-full">
                        {donation.status}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-[11px] text-slate-400 dark:text-slate-500 font-medium">
        ...
        {pagination.totalPages > 1 && (
          <div className="flex justify-between px-8 py-4 border-t border-slate-100 dark:border-slate-800">
            <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page === 1}
              className="px-5 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl font-bold disabled:opacity-40">
              ← Sebelumnya
            </button>
